                ...
                script{
                    withAWS(credentials: 'friendly_credentials_aws', region: 'us-east-1') {
                        catchError(buildlResult: 'UNSTABLE'){
                            aws_rest_endpoint=sh "aws apigateway get-rest-apis --query 'items[?name==`FriendlyApi-${env.ENV_NAME}`].id | [0]'", returnStdout: true
                            aws_rest_endpoint=aws_rest_endpoint.trim();
